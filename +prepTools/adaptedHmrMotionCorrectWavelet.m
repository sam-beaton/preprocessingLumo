function [dodWavelet] = adaptedHmrMotionCorrectWavelet(dod,SD,iqr,turnon)

% function dodWavelet = sbPrePhmrMotionCorrectWavelet(dod,SD,iqr)
%
% UI NAME:
...
%
% Edited 11/1/2024 by SLB: silences 'Loading' message
% Edited by Sam Beaton 09/21/2023 to account for data in which the array
% size is larger than the number of samples
...
for ii = 1:length(lstAct)
    ...
    %create variable to store length of DOD - necessary for short
    %recordings on HD arrays where #chans > #sampled values
    lengthDOD = size(dod); 
    lengthDOD = lengthDOD(1);

    dodWavelet(:,idx_ch) = ARSignal(1:lengthDOD); %index for ARSignal different to orig. hmr function
